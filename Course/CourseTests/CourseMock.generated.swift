...
open class LocalNotificationManagerProtocolMock: LocalNotificationManagerProtocol, Mock {
    public init(sequencing sequencingPolicy: SequencingPolicy = .lastWrittenResolvedFirst, stubbing stubbingPolicy: StubbingPolicy = .wrap, file: StaticString = #file, line: UInt = #line) {
        SwiftyMockyTestObserver.setup()
        self.sequencingPolicy = sequencingPolicy
        self.stubbingPolicy = stubbingPolicy
    ...
    open func send(title: String, body: String, delay: TimeInterval) {
        addInvocation(.m_send__title_titlebody_bodydelay_delay(Parameter<String>.value(`title`), Parameter<String>.value(`body`), Parameter<TimeInterval>.value(`delay`)))
		let perform = methodPerformValue(.m_send__title_titlebody_bodydelay_delay(Parameter<String>.value(`title`), Parameter<String>.value(`body`), Parameter<TimeInterval>.value(`delay`))) as? (String, String, TimeInterval) -> Void
		perform?(`title`, `body`, `delay`)
    }


    fileprivate enum MethodType {
        case m_send__title_titlebody_bodydelay_delay(Parameter<String>, Parameter<String>, Parameter<TimeInterval>)

        static func compareParameters(lhs: MethodType, rhs: MethodType, matcher: Matcher) -> Matcher.ComparisonResult {
            switch (lhs, rhs) {
            case (.m_send__title_titlebody_bodydelay_delay(let lhsTitle, let lhsBody, let lhsDelay), .m_send__title_titlebody_bodydelay_delay(let rhsTitle, let rhsBody, let rhsDelay)):
				var results: [Matcher.ParameterComparisonResult] = []
				results.append(Matcher.ParameterComparisonResult(Parameter.compare(lhs: lhsTitle, rhs: rhsTitle, with: matcher), lhsTitle, rhsTitle, "title"))
        ...
        func intValue() -> Int {
            switch self {
            case let .m_send__title_titlebody_bodydelay_delay(p0, p1, p2): return p0.intValue + p1.intValue + p2.intValue
            }
        }
        func assertionName() -> String {
            switch self {
            case .m_send__title_titlebody_bodydelay_delay: return ".send(title:body:delay:)"
            }
        }
    }
    ...
    public struct Verify {
        ...
        public static func send(title: Parameter<String>, body: Parameter<String>, delay: Parameter<TimeInterval>) -> Verify { return Verify(method: .m_send__title_titlebody_bodydelay_delay(`title`, `body`, `delay`))}
    }
    ...
    public struct Perform {
        ...
        public static func send(title: Parameter<String>, body: Parameter<String>, delay: Parameter<TimeInterval>, perform: @escaping (String, String, TimeInterval) -> Void) -> Perform {
            return Perform(method: .m_send__title_titlebody_bodydelay_delay(`title`, `body`, `delay`), performs: perform)
        }
    }
