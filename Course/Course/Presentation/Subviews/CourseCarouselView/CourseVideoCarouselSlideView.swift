...
struct CourseVideoCarouselSlideView: View {
    ...
    private var totalVideosCount: Int {
        guard let structure = videoContentData.courseVideosStructure else { return 0 }
        return structure.childs.flatMap { getAllVideos(from: $0) }.count
    }
    
    private var completedVideosCount: Int {
        guard let structure = videoContentData.courseVideosStructure else { return 0 }
        return structure.childs.flatMap { getAllVideos(from: $0) }
            .filter { $0.completion >= 1.0 }.count
    }
    ...
    private var videosCompletedView: some View {
        VStack(alignment: .leading, spacing: 8) {
            let total = totalVideosCount
            let completed = completedVideosCount
            if total > 0 {
